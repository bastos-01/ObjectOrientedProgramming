...
public class POO_P_S2 {
	...
	public void alinea2() {
		Cruzeiro cruz = criarCruzeiro(); // coloque o código da alínea 2 aqui
		try {
			PrintWriter output = new PrintWriter(new File("SeaPrincess-2017.txt"));
			output.printf("%s %.1f%s","Percentagem de cabines disponíveis: " ,cruz.percentagemCabines(), "%");
			output.println();
			output.printf("%s %.1f \n","Média de pessoas por quarto nas suites ocupadas: " ,cruz.mediaPessoas());
			output.println("\nLista de cabines ocupadas: \n");
			for(Cabine c: cruz.getConjuntoCabines()) {
			...
			System.err.println("Não foi possível criar o ficheiro!");
		}
	}
